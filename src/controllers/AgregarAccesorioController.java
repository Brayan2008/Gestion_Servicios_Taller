...
public class AgregarAccesorioController {

    private final AccesoriosService ser;

    AgregarAccesorioView av;
    JLabel title;
    ...
    public AgregarAccesorioController(AccesoriosService ser, int opcion) {
        this.av = new AgregarAccesorioView(null);
        ...
        this.tabla = AccesorioViewController.table;
    ...
    protected void addListener() {
        ...
        if (opcion == 2 || opcion == 1) {
            txtCodigo.setText(tabla.getValueAt(AccesorioViewController.row, 0).toString().trim());
            txtCodigo.setFont(Colors.SubTitles);
            txtCodigo.setEditable(false);

            txtNombre.setText(tabla.getValueAt(AccesorioViewController.row, 1).toString());
            //txtPrecio.setText(tabla.getValueAt(AccesorioViewController.row, 2).toString());
    ...
    public void eliminarServicio() {
        var codigo = tabla.getValueAt(AccesorioViewController.row, 0).toString().trim();
        try {
            ser.eliminarCliente(codigo);
            JOptionPane.showMessageDialog(av, "Accesorio eliminado correctamente.");
            ser.buscarCliente("");
            av.dispose();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    protected void guardarServicio() {
        ...
        try {
            ...
            if (opcion == 1) {
                ser.actualizarCliente(cod, nombre);
                JOptionPane.showMessageDialog(av, "Accesorio actualizado correctamente.");
            } else {
                ser.insertarServicio(cod, nombre);
                JOptionPane.showMessageDialog(av, "Accesorio agregado correctamente.");
            }
            ser.buscarCliente("");
            av.dispose();
