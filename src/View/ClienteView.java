...
public class ClienteView implements ConsoleTools {
    ...
    public ClienteView() {
    String opcion; 
        do{ System.out.println("\n".repeat(50) + BOLD + CURVE + "--------------- GESTION DE CLIENTES ------------------- ");
    ...
    public void cambiarDatosCliente() {
        ...
        if ( cliente_find != null) {
                ...
                printInstruction("Nombre (Actual): " + cliente_find.getNombre() + "\tNombre (Modificacion): "); String nombre = lector.nextLine();
            printInstruction("Telefono (Actual): " + cliente_find.getTelefono() + "\tTelefono (Modificacion): "); String tel = lector.nextLine();
                printInstruction("Direccion (Actual): " + cliente_find.getDireccion() + "\tDireccion (Modificacion): "); String direccion = lector.nextLine();
                printInstruction("Distrito (Actual): " + cliente_find.getDistritoCliente() + "\tDistrito (Modificacion): "); String distrito = lector.nextLine();
                printlnInfo("Presione Enter para guardar");
                Cliente cliente_actualizado = new Cliente(dni_buscar, nombre, tel, direccion, cliente_find.getTipoDocumento(), distrito);
    ...
    public void eliminarDatos() {
        ...
        if (button.equals("1")) {
            ...
            verListaClientes();
            printlnInfo("Seleccione a un cliente por su ID para eliminarlo: ");
            String dni_buscar = lector.nextLine();
            Cliente cliente_find = cs.getByID(dni_buscar);
            if ( cliente_find != null) {
                printlnInfo("Se encontro al siguiente cliente: \n");
                imprimirCliente(dni_buscar, cliente_find);
                printlnTitle_Red("¿Desea eliminarlo? s/n");
                String option = lector.nextLine();
                switch (option) {
                    case "s":
                        cs.delete(dni_buscar);
                        printlnInfo("Se elimino correctamente");                
                        break;
                    case "n":
                        printlnInfo("Se cancelo");
                        break;                
                    default:
                        printlnInfo("Opcion Incorrecta. Se cancela la operacion ...");
                        break;
                }
            }else{
                printlnError("El cliente no existe");
            }
        }
        printlnInfo("Presione Enter para volver al menu");
        lector.nextLine();
    }
    
    // Posible candidato a Cliente Service (para arreglar ....)
